...
export default function TshanglaRecorderPage() {
  ...
  const fetchNextSentence = useCallback(async (spk: string, skipIds: number[] = []) => {
    ...
    try {
      ...
      const res = await fetch(`/api/tshangla/sentences?speaker=${encodeURIComponent(spk)}${skipParam}`)
  ...
  const skip = () => {
    ...
    localStorage.setItem('tshangla_skipped_ids', JSON.stringify(newSkippedIds))
    setSkipped((s) => s + 1)
  ...
  const logout = () => {
    ...
    localStorage.removeItem('tshangla_skipped_ids')
    setSpeaker('')
    setSpeakerInput('')
    setSentence(null)
    setSkippedIds([])
  ...
  useEffect(() => {
    ...
    const savedSkippedIds: number[] = JSON.parse(localStorage.getItem('tshangla_skipped_ids') || '[]')
