...
export default function KurtapRecorderPage() {
  ...
  const fetchCount = useCallback(async (spk: string) => {
    const res = await fetch(`/api/kurtap/recordings?speaker=${encodeURIComponent(spk)}`)
    const data = await res.json()
  ...
  const save = async () => {
    ...
    try {
      ...
      const upRes = await fetch('/api/kurtap/upload', { method: 'POST', body: form })
      const upData = await upRes.json()
      if (!upRes.ok) throw new Error(upData.error)

      const recRes = await fetch('/api/kurtap/recordings', {
        method: 'POST',
  ...
  useEffect(() => {
    if (speaker) {
      localStorage.setItem('kurtap_speaker', speaker)
    } else {
      localStorage.removeItem('kurtap_speaker')
    }
  }, [speaker])
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-md">
          <div className="mb-10 text-center">
            ...
            <p className="text-neutral-500 text-sm">Translate English → Record in Kurtap</p>
          ...
          <div className="bg-[#121212] border border-[#222222] rounded-xl p-5 sm:p-8">
            ...
            <input
              ...
              placeholder="e.g. Tshering, Karma..."
          ...
          <p className="text-center text-neutral-700 text-xs mt-6">
            English sentences · Translate then record in Kurtap · Saved to Supabase
          </p>
        </div>
      </div>
    )
      ...
      <header className="border-b border-[#222222] px-4 py-3 sm:px-6 sm:py-4 flex items-center justify-between">
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span className="text-white font-medium text-xs sm:text-sm">Kurtap TTS</span>
          </div>
        </div>
          ...
          <div className="w-full space-y-4">
            ...
            <div className="bg-[#121212] border border-[#222222] rounded-xl p-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-neutral-500 text-[10px] font-bold uppercase tracking-widest">Kurtap Translation</span>
              </div>
              <textarea
                ...
                placeholder="Type your Kurtap translation here..."
                rows={3}
